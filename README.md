...
| TLS 인증서 (prod) | 운영 호스트 | `/appdata/certs/vip/sso/{fullchain,privkey}.pem` (compose 가 :ro 마운트) |
| IdP 서명 인증서 (prod) | 운영 호스트 | `/appdata/certs/vip/idp/saml-signing.pem` (compose 가 :ro 마운트) |
| TLS 인증서 (dev) | **`vision-infra`** | `nginx/certs/{fullchain,privkey}.pem` (`cert-init.sh` 가 self-signed 자동 생성) |
...
nginx/
  ...
  cert-init.sh             # 인증서 부재 시 self-signed 페어 자동 생성 (dev 전용 폴백)
  certs/                   # dev TLS 인증서 (.gitignore — cert-init.sh 가 관리). prod 는
                           # /appdata/certs/vip/sso 에서 직접 마운트.
db/
...
3. `/appdata/certs/vip/sso/{fullchain,privkey}.pem` 에 실제 TLS 인증서가 있는지
   확인. prod 에서는 :ro 로 마운트되므로 누락 시 `nginx-cert-init` 이 fail-fast
   (self-signed 폴백 없음). dev 에서는 `cert-init.sh` 가 self-signed 를 자동 생성.
...
# 3. 레지스트리 로그인 (1회):
#       docker login "$REGISTRY_URL" -u "$REGISTRY_USERNAME" -p "$REGISTRY_PASSWORD"
# 4. 인증서 배치 — docker-compose.prod.yml 이 세 경로를 전부 :ro 로 마운트.
